...
class BodyPart:
    ...
    def get_human_id(self):
        return int(self.uidx.split("-")[0])

    def get_coordinates(self):
        return [self.part_idx, self.x, self.y]

    def __str__(self):
        return "Human:%s BodyPart:%d-(%.2f, %.2f) score=%.2f" % (
            self.uidx,
            self.part_idx,
            self.x,
            self.y,
    ...
    def __init__(
        ...
        with graph_mode():
            ...
            if trt_bool is True:
                ...
                conversion_params = conversion_params._replace(maximum_cached_engines=int(1e3))
                ...
                converter = trt.TrtGraphConverterV2(
                    input_saved_model_dir=graph_path,
                    conversion_params=conversion_params)
                converter.convert()
                #converter.save(output_nodes)
        ...
        with graph_mode():
            ...
            print("HEEEEJ")
            print(self.tensor_output[:, :, :, :19])

            self.upsample_size = tf.compat.v1.placeholder(
                dtype=tf.int32, shape=(2,), name="upsample_size"
            )
            self.tensor_heatMat_up = tf.compat.v1.image.resize_area(
                self.tensor_output[:, :, :, :19],
                self.upsample_size,
                align_corners=False,
                name="upsample_heatmat",
            )
            self.tensor_pafMat_up = tf.compat.v1.image.resize_area(
                self.tensor_output[:, :, :, 19:],
                self.upsample_size,
                align_corners=False,
